    ...
    function UPDATEMOVEMENT() {
        connection.execute("SELECT has_moved FROM playermatch WHERE player_id = ?", [player_id], (err, results) => {
            ...
            connection.execute('SELECT tile_id, player_hazard_duration FROM playermatch WHERE player_id = ?', [player_id], (err, playermatchResults, fields) => {
                ...
                if (
                    player_tile_id + 1 == tile_id ||
                    player_tile_id - 1 == tile_id ||
                    player_tile_id + 5 == tile_id ||
                    player_tile_id - 5 == tile_id
                ) {
                    getTileTypeId(tile_id, (err, tileType) => {
                        ...
                        connection.execute('UPDATE playermatch SET tile_id = ?, has_moved = TRUE WHERE player_id = ?', [tile_id, player_id], (err, updateResult) => {
                            if (err) {
                            ...
                            res.status(200).send("Player moved successfully and has_moved value updated");
                        });
                    });
                } else {
...
app.put("/endturn", (req, res) => {
    ...
    function UpdateData(){
        connection.execute('UPDATE playermatch SET has_moved = FALSE WHERE 1', [], (err, results) => {
            if (err) {
    ...
    function ChangeTurn(){
        ...
        connection.execute("select player_id from playermatch WHERE match_id = 1", [], (err, results, fields) => {
            ...
            if (results.length < 2){
                res.status(500).send("There should be 2 players inside this match."); 
                return;
            }
